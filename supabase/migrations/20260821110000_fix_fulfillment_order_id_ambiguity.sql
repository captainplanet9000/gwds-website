-- Keep the current production function aligned with the clean-replay commerce
-- migration. The output column is named order_id, so PL/pgSQL must prefer SQL
-- columns when a statement also references a table column with that name.
