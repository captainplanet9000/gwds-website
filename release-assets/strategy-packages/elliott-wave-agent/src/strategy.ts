...
export interface StrategyConfig {
  swingLookback: number;           // Periods for pivot detection (default: 5)
  minWaveSize: number;              // Minimum wave size as % (default: 1.5%)
  maxWaveDeviation: number;         // Max deviation from ideal Fibonacci ratio % (default: 15%)
  wave3MinMultiplier: number;       // Wave 3 minimum vs wave 1 (default: 1.0)
  wave3IdealMultiplier: number;     // Wave 3 ideal Fibonacci ratio (default: 1.618)
  wave5IdealRatio: number;          // Wave 5 vs wave 1 ratio (default: 1.0)
  fibRetrace618: number;            // 61.8% Fib level (default: 0.618)
  fibRetrace50: number;             // 50% Fib level (default: 0.5)
  fibRetrace382: number;            // 38.2% Fib level (default: 0.382)
  minConfidence: number;            // Minimum confidence to signal (default: 60)
  degreeLabels: string[];           // Wave degrees (default: ['Minute', 'Minor', 'Intermediate'])
}
 ...
 */
function fibRetrace(start: number, end: number, level: number): number {
...
): { valid: boolean; confidence: number; violations: string[] } {
 ...
 */
function identifyImpulsePattern(
  swings: SwingPoint[],
  config: StrategyConfig,
  currentPrice: number
): ImpulsePattern | null {
  ...
  for (let i = swings.length - 1; i >= 5; i--) {
    ...
    if (swings[i].type !== 'high') continue; // Wave 5 end
    if (swings[i-1].type !== 'low') continue;  // Wave 4 end
    if (swings[i-2].type !== 'high') continue; // Wave 3 end
    if (swings[i-3].type !== 'low') continue;  // Wave 2 end
    if (swings[i-4].type !== 'high') continue; // Wave 1 end
    if (swings[i-5].type !== 'low') continue;  // Wave 1 start
    ...
    if (Math.abs(wave1.sizePercent) < config.minWaveSize) continue;
    if (Math.abs(wave3.sizePercent) < config.minWaveSize) continue;
...
function checkWaveCompletion(
  ...
  currentPrice: number,
  config: StrategyConfig
): { atWave2: boolean; atWave4: boolean; wave?: Wave; impulse?: ImpulsePattern } {
  
  if (swings.length < 4) return { atWave2: false, atWave4: false };
