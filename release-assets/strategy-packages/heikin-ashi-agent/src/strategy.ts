...
export interface StrategyConfig {
  smoothingPeriod: number;          // Apply HA transformation N times (default: 1, use 2 for extra smoothing)
  minConsecutiveCandles: number;    // Min consecutive same-color candles for trend (default: 3)
  wickFilter: boolean;              // Require wickless candles for strong trend (default: true)
  emaConfirmation: boolean;         // Confirm with EMA crossover (default: true)
  emaPeriod: number;                // EMA period for confirmation (default: 20)
  trendStrengthThreshold: number;   // Min consecutive candles for high confidence (default: 5)
  dojiThreshold: number;            // Max body size % to be doji (default: 0.5%)
  minBodySize: number;              // Min body size % for valid signal (default: 0.3%)
  stopLossATR: number;              // Stop loss in ATR multiples (default: 2.0)
  takeProfitRatio: number;          // R:R ratio (default: 2.5)
}
 ...
 */
function transformToHeikinAshi(candles: Candle[], dojiThreshold: number): HeikinAshiCandle[] {
 ...
 */
function smoothHeikinAshi(candles: Candle[], iterations: number, dojiThreshold: number): HeikinAshiCandle[] {
  ...
  for (let iter = 1; iter < iterations; iter++) {
    const tempCandles: Candle[] = haCandles.map(ha => ({
      open: ha.open,
      high: ha.high,
      low: ha.low,
      close: ha.close,
 ...
 */
function countConsecutiveCandles(haCandles: HeikinAshiCandle[]): { color: 'green' | 'red'; count: number } {
  if (haCandles.length === 0) return { color: 'green', count: 0 };
  
  const currentColor = haCandles[haCandles.length - 1].color;
  if (currentColor === 'doji') return { color: 'green', count: 0 };
  ...
  return { color: currentColor as 'green' | 'red', count };
}
 ...
 */
function detectStrongTrend(haCandles: HeikinAshiCandle[], minConsecutive: number): boolean {
  const consecutive = countConsecutiveCandles(haCandles);
  if (consecutive.count < minConsecutive) return false;
...
export const strategy = {
  ...
  execute(candles: Candle[], config?: Partial<StrategyConfig>): StrategySignal {
    ...
    if (
      ...
      let confidence = 60;
      if (emaConfirmed) confidence += 15;
      if (!currentHA.hasLowerWick) confidence += 10; // Strong bullish candle
      if (currentHA.bodyPercent > 1.5) confidence += 5; // Large body
    ...
    if (
      ...
      let confidence = 60;
      if (emaConfirmed) confidence += 15;
      if (!currentHA.hasUpperWick) confidence += 10; // Strong bearish candle
      if (currentHA.bodyPercent > 1.5) confidence += 5;
