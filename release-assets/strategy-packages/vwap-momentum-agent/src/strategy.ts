...
export interface StrategyConfig {
  sessionLength: number;           // Session period in candles (default: 24 for daily reset in hourly)
  bandMultiplier: number[];         // Std dev multipliers (default: [1, 2, 3])
  volumeThreshold: number;          // Volume surge multiplier (default: 2.0)
  cumulativeDeltaPeriod: number;   // Periods for delta calc (default: 20)
  pocBinSize: number;               // Price bins for volume profile % (default: 0.5%)
  anchoredPeriod: number;           // Lookback for anchored VWAP (default: 50)
  meanReversionBand: number;        // Band level for mean reversion (default: 2)
  minSessionVolume: number;         // Min avg volume for valid signals (default: 100000)
  priceMovementThreshold: number;   // Min % move for breakout (default: 0.5%)
}
...
interface VolumeProfile {
  ...
  bins: Map<number, number>; // Price -> Volume mapping
}

/**
 * Calculate VWAP and standard deviation bands
 */
function calculateVWAP(candles: Candle[], sessionLength: number, bandMultipliers: number[]): VWAPData {
 ...
 */
function calculateAnchoredVWAP(candles: Candle[], anchorIndex: number): number {
 ...
 */
function findRecentSwingLow(candles: Candle[], lookback: number): number {
 ...
 */
function calculateCumulativeDelta(candles: Candle[], period: number): number {
 ...
 */
function calculateVolumeProfile(candles: Candle[], sessionLength: number, binSizePercent: number): VolumeProfile {
  const sessionCandles = candles.slice(-sessionLength);
 ...
 */
function hasVolumeSurge(currentVolume: number, recentCandles: Candle[], threshold: number): boolean {
...
export const strategy = {
  ...
  execute(candles: Candle[], config?: Partial<StrategyConfig>): StrategySignal {
    ...
    if (!previousAboveVWAP && currentAboveVWAP && volumeSurge && deltaBias === 'bullish' && vwapData.upperBands[1] > currentPrice) {
      ...
      let confidence = 70;
      if (currentPrice > anchoredVWAP) confidence += 10; // Aligned with anchored VWAP
      if (currentPrice > volumeProfile.poc) confidence += 5; // Above POC
      if (cumulativeDelta > vwapData.totalVolume * 0.1) confidence += 10; // Strong buying
    ...
    if (previousAboveVWAP && !currentAboveVWAP && volumeSurge && deltaBias === 'bearish' && vwapData.lowerBands[1] > 0 && vwapData.lowerBands[1] < currentPrice) {
      ...
      let confidence = 70;
      if (currentPrice < anchoredVWAP) confidence += 10;
      if (currentPrice < volumeProfile.poc) confidence += 5;
      if (cumulativeDelta < -(vwapData.totalVolume * 0.1)) confidence += 10;
