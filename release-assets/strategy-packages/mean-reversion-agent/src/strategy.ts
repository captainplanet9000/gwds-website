...
export interface StrategyConfig {
  bbPeriod: number;                 // Bollinger Band SMA period (default: 20)
  bbStdDev: number;                 // Standard deviation multiplier (default: 2.0)
  kcPeriod: number;                 // Keltner Channel EMA period (default: 20)
  kcMultiplier: number;             // ATR multiplier for KC (default: 1.5)
  ...
  bandwidthThreshold: number;       // Squeeze threshold % (default: 0.05 = 5%)
  walkingThreshold: number;         // Periods at band to consider trending (default: 3)
  doubleBottomLookback: number;     // Periods to check for double bottom (default: 20)
  minBodyPercent: number;           // Min candle body % for valid signal (default: 0.3%)
  takeProfitTarget: 'middle' | 'opposite';  // TP at middle band or opposite band (default: 'middle')
}
 ...
 */
function calculateBollingerBands(closes: number[], period: number, stdDevMult: number, currentPrice: number): BollingerBands {
 ...
 */
function calculateKeltnerChannels(candles: Candle[], period: number, multiplier: number): KeltnerChannels {
 ...
 */
function detectSqueeze(bb: BollingerBands, kc: KeltnerChannels): boolean {
 ...
 */
function isWalkingBands(candles: Candle[], bb: BollingerBands, walkingThreshold: number): { walking: boolean; direction?: 'up' | 'down' } {
 ...
 */
function detectDoubleBottom(candles: Candle[], bb: BollingerBands, lookback: number): boolean {
  const recentCandles = candles.slice(-lookback);
  
  // Find lows that touched lower band
  const bandTouches: number[] = [];
  ...
  if (bandTouches.length < 2) return false;
 ...
 */
function detectDoubleTop(candles: Candle[], bb: BollingerBands, lookback: number): boolean {
  const recentCandles = candles.slice(-lookback);
...
export const strategy = {
  ...
  execute(candles: Candle[], config?: Partial<StrategyConfig>): StrategySignal {
    ...
    if (
      ...
      let confidence = 65;
      if (doubleBottom) confidence += 15; // Double bottom is strong
      if (rsi < 25) confidence += 10; // Extremely oversold
      if (bb.percentB < 0) confidence += 5; // Below lower band
      if (squeeze) confidence += 5; // Squeeze = potential expansion
    ...
    if (
      ...
      let confidence = 65;
      if (doubleTop) confidence += 15;
      if (rsi > 75) confidence += 10;
      if (bb.percentB > 1) confidence += 5;
      if (squeeze) confidence += 5;
    ...
    let position = 'middle';
    if (bb.percentB > 0.8) position = 'upper zone';
    else if (bb.percentB < 0.2) position = 'lower zone';
    
    let rsiZone = 'neutral';
    if (rsi > cfg.rsiOverbought) rsiZone = 'overbought';
    else if (rsi < cfg.rsiOversold) rsiZone = 'oversold';
