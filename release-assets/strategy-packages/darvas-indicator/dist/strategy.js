 ...
 */
function findPivotHighs(candles, lookback) {
    const pivots = [];
 ...
 */
function findPivotLows(candles, lookback) {
    const pivots = [];
    for (let i = lookback; i < candles.length - lookback; i++) {
 ...
 */
function identifyBoxes(candles, config) {
    const boxes = [];
    ...
    for (const pivot of pivotHighs) {
        ...
        if (subsequentCandles.length < config.boxConfirmationPeriod)
            continue;
        ...
        // Validate box height
        if (boxHeight < config.minBoxHeight)
            continue;
        ...
        if (!confirmed || confirmedAt >= candles.length)
            continue;
 ...
 */
function checkBreakout(currentCandle, boxes, config, recentCandles) {
    if (boxes.length === 0) {
 ...
 */
function checkBreakdown(currentCandle, boxes) {
    if (boxes.length === 0)
        return false;
    const activeBox = boxes[0];
...
exports.strategy = {
    ...
    execute(candles, config) {
        ...
        if (breakoutCheck.isBreakout && breakoutCheck.box) {
            ...
            if (breakoutCheck.volumeConfirmed)
                confidence += 25;
