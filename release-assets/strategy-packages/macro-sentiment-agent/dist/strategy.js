 ...
 */
function estimateFundingRate(candles, period) {
 ...
 * Real implementation would fetch from exchange API
 */
function estimateOpenInterest(candles, period) {
    const recentCandles = candles.slice(-period);
 ...
 */
function calculateOIChange(candles, period) {
 ...
 * Real implementation would fetch from exchange API
 */
function estimateLongShortRatio(candles, period) {
    const recentCandles = candles.slice(-period);
    let bullishVolume = 0;
    let bearishVolume = 0;
    for (const candle of recentCandles) {
 ...
 * - Long/short ratio (30%)
 */
function calculateFearGreedIndex(candles, period) {
    const recentCandles = candles.slice(-period);
 ...
 */
function detectLiquidationCascade(candles, volumeMultiplier, priceThreshold) {
    if (candles.length < 5)
        return false;
 ...
 */
function estimateStablecoinFlow(candles, period) {
 ...
 */
function simulateBTCDominance(candles) {
 ...
 */
function gatherSentimentData(candles, config) {
    return {
...
exports.strategy = {
    ...
    execute(candles, config) {
        ...
        if (sentiment.fearGreedIndex < cfg.extremeFear &&
            ...
            let confidence = 70;
            if (sentiment.fearGreedIndex < 10)
                confidence += 10; // Extreme panic
            if (sentiment.liquidationRisk > 0)
                confidence += 10; // Liquidation flush
            if (sentiment.stablecoinFlow > 10)
                confidence += 5; // Strong inflows
            return {
                direction: 'long',
        ...
        if (sentiment.fearGreedIndex > cfg.extremeGreed &&
            sentiment.fundingRate > cfg.fundingThreshold &&
            sentiment.stablecoinFlow < 0) {
            const entry = currentPrice;
            ...
            let confidence = 70;
            if (sentiment.fearGreedIndex > 90)
                confidence += 10; // Extreme euphoria
            if (sentiment.longShortRatio > 3)
                confidence += 10; // Overleveraged longs
            if (sentiment.stablecoinFlow < -10)
                confidence += 5; // Outflows
            return {
                direction: 'short',
                confidence: Math.min(95, confidence),
        ...
        if (sentiment.oiChange > cfg.oiChangeThreshold &&
        ...
        if (sentiment.fearGreedIndex < 40)
            sentimentZone = 'fear';
        else if (sentiment.fearGreedIndex > 60)
            sentimentZone = 'greed';
        let fundingBias = 'balanced';
        if (sentiment.fundingRate > cfg.fundingThreshold)
            fundingBias = 'long-heavy';
        else if (sentiment.fundingRate < -cfg.fundingThreshold)
