...
export async function sendHostingEmail(email: string, message: HostingEmailData) {
  ...
  const copy = {
    ...
    hosting_wallet_needed: [message.title || 'Verify your wallet to finish setup', message.detail || 'Your subscription is active, but dashboard setup is waiting for wallet ownership verification. Open your hosting account, connect the wallet you want to use, and sign the ownership message. This does not transfer funds or enable trading.'],
    hosting_resumed: [message.title || 'Your hosted workspace has resumed', message.detail || 'Your workspace is running again. Open your hosting account to review health and trading controls before using agents.'],
  }[message.template];
